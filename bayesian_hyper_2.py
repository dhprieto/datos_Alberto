...
def fit_lightgbmc(X_train, y_train, X_test, y_test,
    ...
    opt = BayesSearchCV(
        pipeline,
        {
            'LGBMC__n_estimators': [ int(x) for x in range(10,510) ],
            'LGBMC__max_depth': [ int(x) for x in range(1,10) ],
            'LGBMC__learning_rate': (1e-4, 1e-1, 'uniform'),
            'LGBMC__subsample': (.01, 1, 'uniform'),
            'LGBMC__boosting_type': ['gbdt', 'dart'],
            'LGBMC__num_leaves': [ int(x) for x in range(1024,2000) ]
...
def fit_SGD(X_train, y_train, X_test, y_test,
    ...
    opt = BayesSearchCV(
        pipeline,
        {
            ...
            "SGD__tol" : (1e-5, 1e-2, 'log-uniform')
...
def fit_XGB(X_train, y_train, X_test, y_test,
    ...
    opt = BayesSearchCV(
        pipeline,
        {
            'XGB__n_estimators': list(range(100, 1000)),
...
def fit_XGBRFC(X_train, y_train, X_test, y_test,
    ...
    opt = BayesSearchCV(
        pipeline,
        {
            ...
            'XGBRFC__max_depth': (1, 50, 'log-uniform'),
            'XGBRFC__reg_lambda': (1e-5, 1e+5, 'log-uniform'),
            'XGBRFC__reg_alpha': (1e-5, 1e+5, 'log-uniform'),
            'XGBRFC__gamma': (1e-5, 1e+5, 'log-uniform'),
...
def main(first_order=False, second_order=True):
